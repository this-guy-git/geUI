...
export const ThemeXbox: React.FC<{ config: Config; selectedEmu: number; setSelectedEmu: (i: number) => void }> = ({ config, selectedEmu, setSelectedEmu }) => {
  ...
  return (
    <div style={{
      background: 'linear-gradient(135deg, #107c10 60%, #1d9f1d 100%)',
      color: '#fff',
      minHeight: '100vh',
      padding: 32,
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 32 }}>
        {config.emulators.map((emu, i) => (
          <button
            ...
            style={{
              padding: '18px 32px',
              background: i === selectedEmu ? 'linear-gradient(90deg,#1d9f1d,#107c10)' : '#222',
              color: '#fff',
              border: 'none',
              borderRadius: 16,
              fontSize: 20,
              fontWeight: 500,
              boxShadow: i === selectedEmu ? '0 2px 12px #1d9f1d88' : 'none',
              cursor: 'pointer',
      ...
      <div style={{
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, 228px)', gap: 24, justifyContent: 'center', justifyItems: 'center' }}>
            {games.map((game, idx) => (
              <GameCard key={idx} game={game} cardBackground="#107c10" />
            ))}
          </div>
        )}
      </div>
      <div style={{ color: '#eee', fontSize: 16 }}>Games directory: <span style={{ color: '#fff' }}>{emu.gamesDir}</span></div>
    </div>
  );
};
